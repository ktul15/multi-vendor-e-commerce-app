...
const bodySchema = z.object({ action: z.enum(["activate", "deactivate", "delete"]) });
...
export async function PATCH(
  ...
  if (!uuidPattern.test(id)) return failure("Invalid product ID", 400);
  ...
  const endpoint =
    action === "delete"
      ? `${apiBase}/admin/products/${id}`
      : `${apiBase}/admin/products/${id}/${action}`;
  try {
