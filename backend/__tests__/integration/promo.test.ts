...
beforeAll(async () => {
  ...
  await prisma.user.upsert({
    ...
    create: {
      name: 'Admin User',
      email: 'admin.promo@ecommerce.com',
      password,
      role: 'ADMIN',
      isVerified: true,
    },
  ...
  const customer = await prisma.user.upsert({
    ...
    create: {
      name: 'Promo Customer',
      email: 'customer.promo@ecommerce.com',
      password,
      role: 'CUSTOMER',
      isVerified: true,
    },
  ...
  const vendor = await prisma.user.upsert({
    ...
    create: {
      name: 'Promo Vendor',
      email: 'vendor.promo@ecommerce.com',
      password,
      role: 'VENDOR',
      isVerified: true,
    },
  ...
  await prisma.vendorProfile.upsert({
    ...
    create: {
      userId: vendor.id,
      storeName: 'Promo Integration Store',
      status: 'APPROVED',
  ...
  const adminLogin = await request(app)
    ...
    .send({ email: 'admin.promo@ecommerce.com', password: 'test1234' });
  adminToken = `Bearer ${adminLogin.body.data.tokens.accessToken}`;

  const customerLogin = await request(app)
    .post('/api/v1/auth/login')
    .send({ email: 'customer.promo@ecommerce.com', password: 'test1234' });
  customerToken = `Bearer ${customerLogin.body.data.tokens.accessToken}`;

  const vendorLogin = await request(app)
    .post('/api/v1/auth/login')
    .send({ email: 'vendor.promo@ecommerce.com', password: 'test1234' });
...
describe('Promo Code Admin CRUD', () => {
  ...
  describe('DELETE /api/v1/promo-codes/:id', () => {
    it('should soft delete and hide promo code from lists', async () => {
      ...
      // Verify soft-deleted promos are not available for editing/fetching.
      ...
      const listRes = await request(app)
        .get('/api/v1/promo-codes?search=DELETEME')
        .set('Authorization', adminToken);
