...
function request(body: unknown, csrf = true) {
  return new NextRequest(`https://admin.test/api/users/${userId}/status`, {
...
describe("admin user status BFF", () => {
  it("rejects missing CSRF, invalid IDs, and unsupported actions before mutation", async () => {
    const fetch = vi.fn();
    vi.stubGlobal("fetch", fetch);

    expect((await PATCH(request({ action: "ban" }, false), params)).status).toBe(403);
    expect(
      (
        await PATCH(request({ action: "ban" }), {
  ...
  it("forwards a validated action and preserves forbidden transitions", async () => {
    const fetch = vi
      .fn<typeof globalThis.fetch>()
      .mockResolvedValueOnce(
        Response.json({ data: admin, message: "Admin retrieved", success: true }),
      )
      .mockResolvedValueOnce(
        Response.json({ message: "Cannot ban an admin account", success: false }, { status: 403 }),
      );
    vi.stubGlobal("fetch", fetch);

    const response = await PATCH(request({ action: "ban" }), params);
    expect(response.status).toBe(403);
    expect(fetch.mock.calls[1]?.[0]).toBe(`https://api.test/api/v1/admin/users/${userId}/ban`);
