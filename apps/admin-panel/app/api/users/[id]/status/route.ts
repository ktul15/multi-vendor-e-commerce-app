...
const bodySchema = z.object({ action: z.enum(["ban", "unban"]) });
...
export async function PATCH(
  ...
  if (!uuidPattern.test(id)) return failure("Invalid user ID", 400);
  ...
  if (!token || !apiBase) return finalize(failure("User service unavailable", 503));
  try {
    const response = await fetch(`${apiBase}/admin/users/${id}/${parsed.data.action}`, {
    ...
    return finalize(failure("User service unavailable", 503));
  }
}
