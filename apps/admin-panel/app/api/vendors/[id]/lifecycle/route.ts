...
const bodySchema = z.object({ action: z.enum(["approve", "reject", "suspend"]) });
...
export async function PATCH(
  ...
  if (!token || !apiBase) return finalize(failure("Vendor service unavailable", 503));
  try {
    const response = await fetch(`${apiBase}/admin/vendors/${id}/${parsed.data.action}`, {
    ...
    return finalize(failure("Vendor service unavailable", 503));
  }
}
