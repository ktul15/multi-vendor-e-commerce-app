...
export class CartService {
    private toCartWithTotals(cart: Prisma.CartGetPayload<{ include: typeof cartInclude }>): CartWithTotals {
    ...
    async addItem(userId: string, input: AddCartItemInput): Promise<CartWithTotals> {
        ...
        const updatedCart = await prisma.$transaction(async (tx) => {
            ...
            if (!variant.product.isActive) throw ApiError.badRequest('This product is no longer available');
    ...
    async updateItem(userId: string, itemId: string, input: UpdateCartItemInput): Promise<CartWithTotals> {
     ...
     */
    async previewPromo(userId: string, code: string): Promise<{
        code: string;
