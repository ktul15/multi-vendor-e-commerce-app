...
Contract correction note: the source-level API audit in `docs/migrations/dashboard-api-contract-audit.md` supersedes response-shape and OpenAPI claims in this earlier parity baseline. Corrections discovered by #74 are incorporated below.
...
| Product delete | Confirmation on `/products` | Warns that deletion cannot be undone, deletes, reloads table | `DELETE /products/{id}` | Approved owning vendor | Confirmation, deleting/disabled, unmapped order-history constraint/server error, success | `/products` | Preserve explicit confirmation. Keep the row stable while pending and restore/report failure clearly; #129/#130 must map the current unknown error to an actionable conflict. |
...
| Order progression | Dialog on `/orders` | Presents the single forward transition as text; requires tracking number/carrier for `SHIPPED`; terminal states show no action | `PUT /orders/vendor/{vendorOrderId}/status` | Approved vendor owning the vendor order | Confirmation, tracking validation, submitting, stale/invalid transition (400), success, terminal | `/orders`; planned `/orders/[id]` after API support | Preserve the forward-only rule. Use a direct contextual action when exactly one transition exists; never show a one-option dropdown. The detail route must not ship until a vendor-owned detail endpoint can hydrate direct loads safely. |
...
| Edit vendor profile | `PUT /vendor-profile/me` | Multipart: optional `storeName`, `description`, `logo`, `banner`; the service rejects a true no-op with 400 and accepts image-only changes. | updated profile | Invalidate profile, shell status, and store query. Runtime media replacement rolls back new uploads on DB failure and deletes replaced assets best effort after commit. |
...
| Read category tree | `GET /categories` | None | roots, children, and grandchildren (three levels) | Cache as shared reference data; #131 must return a complete tree or enforce a documented maximum depth. |
...
| Update product | `PUT /products/{id}` | optional category, descriptive fields, images, tags, price, active flag; current schema accepts `{}` | updated product, including a possible no-op | Invalidate list/detail/catalog; #129/#130 must require and document a meaningful change. |
...
| Update variant | `PUT /products/{id}/variants/{variantId}` | optional variant fields; current schema accepts `{}` | updated variant, including a possible no-op | Invalidate product list/detail/catalog; #129/#130 must require and document a meaningful change. |
...
| Read Connect status | `GET /vendor-payouts/connect/status` | None | `onboardingStatus`, `chargesEnabled`, `payoutsEnabled`, `detailsSubmitted` | Refresh on return and when page regains focus; OpenAPI's `connected` example is stale. |
| List earnings | `GET /vendor-payouts/earnings` | page, limit, optional status/dates | `earnings` and `pagination`; empty results have zero total pages | Key by filters/page; refresh after payment/payout webhook effects. |
| Earnings summary by status | `GET /vendor-payouts/earnings/summary` | None | `pending`, `transferred`, `failed`, `reversed` buckets with count and gross/commission/net amounts | Refresh with earnings and after webhook effects. |
| List payouts | `GET /vendor-payouts/payouts` | page, limit, optional status | `payouts` and `pagination`; empty results have zero total pages | Key by filters/page; refresh after payout webhook effects. |
...
- The Zod body schema accepts an empty object, but the service rejects a true no-op with operational 400 “No fields to update.” Uploaded files count as changes, so image-only updates are accepted.
...
- Product and variant update schemas currently accept empty objects, permitting no-op mutations. #129/#130 must require at least one meaningful changed field and document the validation response.
...
| Product and variant updates accept empty payloads | Runtime schemas permit `{}`, so a client can submit a successful no-op | **Fix under #129/#130.** Require at least one meaningful changed field. |
